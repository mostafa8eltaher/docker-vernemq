...
format_config() {
  while read data; do
    key=$(echo $data | awk -F '=' '{ print $1 }' | tr '[:upper:]' '[:lower:]')
...
env | grep DOCKER_VERNEMQ | grep -v 'DISCOVERY_NODE\|DOCKER_VERNEMQ_USER' | cut -c 16- | format_config | sed 's/__/./g' >> /etc/vernemq/vernemq.conf
